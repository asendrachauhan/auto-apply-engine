...
export class ApiService {
  ...
  getNotifications(params: { page?: number; limit?: number; type?: string; unreadOnly?: boolean } = {}): Observable<ApiResponse> {
    return this.get(`/notifications`, params);
  }
  getUnreadNotificationCount(): Observable<ApiResponse> { return this.get(`/notifications/unread-count`); }
  markNotificationRead(id: string): Observable<ApiResponse> { return this.patch(`/notifications/${id}/read`, {}); }
  markAllNotificationsRead(): Observable<ApiResponse> { return this.patch(`/notifications/read-all`, {}); }
  deleteNotification(id: string): Observable<ApiResponse> { return this.delete(`/notifications/${id}`); }

  // ── Referral (/api/referral) ──────────────────────────────────────────────
  getMyReferral(): Observable<ApiResponse> { return this.get(`/referral/me`); }
  getReferralHistory(params: { page?: number; limit?: number } = {}): Observable<ApiResponse> {
    return this.get(`/referral/history`, params);
  }

  // ── Admin (/api/admin) — requires role: 'admin' ───────────────────────────
  getAdminStats(): Observable<ApiResponse> { return this.get(`/admin/stats`); }
  getAdminUsers(params: { page?: number; limit?: number; search?: string; plan?: string; role?: string } = {}): Observable<ApiResponse> {
    return this.get(`/admin/users`, params);
  }
  getAdminUserDetail(id: string): Observable<ApiResponse> { return this.get(`/admin/users/${id}`); }
  updateAdminUser(id: string, payload: any): Observable<ApiResponse> { return this.patch(`/admin/users/${id}`, payload); }
  getAdminSettings(): Observable<ApiResponse> { return this.get(`/admin/settings`); }
  putAdminSetting(key: string, value: any, category = 'general'): Observable<ApiResponse> {
    return this.put(`/admin/settings/${key}`, { value, category });
  }
  deleteAdminSetting(key: string): Observable<ApiResponse> { return this.delete(`/admin/settings/${key}`); }
}
