...
@Component({
        ...
        <form (ngSubmit)="onSubmit()" class="auth-form">
          ...
          @if (referralCode) {
            <div class="referral-banner">
              <aa-icon name="gift" [size]="14"/> {{ 'AUTH.REFERRAL_APPLIED' | translate: { code: referralCode } }}
            </div>
          }
  ...
  styles: [`
    ...
    .referral-banner { display:flex; align-items:center; gap:8px; padding:10px 14px; margin-bottom:14px; border-radius:10px; background:rgba(168,85,247,.1); color:#a855f7; font-size:12px; font-weight:600; }
...
export class RegisterComponent {
  ...
  constructor(private auth: AuthService, private router: Router, private route: ActivatedRoute, private toast: ToastService, private translate: TranslateService) {
