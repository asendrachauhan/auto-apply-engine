...
const userSchema = new mongoose.Schema({
  ...
  // Referral program — see models/ReferralReward.js for the reward ledger.
  // referralCode is this user's own shareable code; referredBy records
  // whose code they signed up with (if any); referralPoints is their
  // current redeemable balance (credited only on the referred user's
  // first PAID plan purchase, never on signup alone — see
  // routes/subscription.routes.js webhook handler).
...
// ── Pre-save: assign a referral code to new users ────────────────────────────
userSchema.pre('save', async function (next) {
  if (!this.isNew || this.referralCode) return next();
  // Small retry loop to dodge the (very unlikely) collision on the unique index.
  for (let attempt = 0; attempt < 5; attempt++) {
    const candidate = generateReferralCode(this.name);
    const exists = await this.constructor.exists({ referralCode: candidate });
    if (!exists) { this.referralCode = candidate; break; }
  }
  next();
});
