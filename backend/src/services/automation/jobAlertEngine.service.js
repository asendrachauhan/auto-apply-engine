  ...
  try {
    ...
    for (const job of newJobs) {
      ...
      try {
        ...
        const channels = ['in-app', ...(notifResult.email ? ['email'] : []), ...(notifResult.whatsapp ? ['whatsapp'] : [])];
        notifyInApp({
          userId, type: 'job_alert',
          title:   `New match: ${job.title}`,
          message: `${job.company} — ${matchResult.matchScore}% match. Your tailored resume and prefill packet are ready.`,
          channels, link: `/alerts/${alertDoc._id}`,
          metadata: { jobAlertId: alertDoc._id.toString(), matchScore: matchResult.matchScore },
        }).catch(() => {}); // notifyInApp already logs its own failures
