...
/** Admin-only access control — must run after protect. */
const requireAdmin = (req, res, next) => {
  if (req.user?.role !== 'admin') {
    return sendError(res, HTTP.FORBIDDEN, 'Admin access required');
  }
  next();
};

module.exports = { protect, authenticate: protect, requireVerifiedEmail, requirePlan, requireAdmin };
